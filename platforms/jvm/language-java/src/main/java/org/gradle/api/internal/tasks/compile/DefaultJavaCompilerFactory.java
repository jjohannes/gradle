...
public class DefaultJavaCompilerFactory implements JavaCompilerFactory {
    ...
    private <T extends CompileSpec> Compiler<T> createTargetCompiler(Class<T> type, Collection<File> customCompilerClasspath) {
        ...
        if (ForkingJavaCompileSpec.class.isAssignableFrom(type)) {
            return (Compiler<T>) new DaemonJavaCompiler(workingDirProvider.getWorkingDirectory(), JdkJavaCompiler.class, new Object[]{getJavaHomeBasedJavaCompilerFactory(customCompilerClasspath)}, new ProcessIsolatedCompilerWorkerExecutor(workerDaemonFactory, actionExecutionSpecFactory, projectCacheDir), forkOptionsFactory, classPathRegistry);
        } else {
            return (Compiler<T>) new JdkJavaCompiler(getJavaHomeBasedJavaCompilerFactory(customCompilerClasspath), problems);
        }
    }
}
